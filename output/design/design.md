...
| 数据回流管理 | 超管发起数据回流申请，选择对接方式（开发者提供接口/开发者主动推送），开发者响应（上传对接文档或查看数据）；联调完成标记、数据回流明细台账 | 桂教通超管、开发者管理员 |
...
| 桂教通超管 | 管理全平台能力：开发者管理、接口管理、应用管理（P28All，查看所有厂商应用并执行接口授权/数据授权/取消授权）、客户端授权管理、接口申请审批（P08，位于审批管理下）、组织同步审批（P41，审批机构管理员提交的组织同步申请，通过后自动生成客户端授权记录）、运营统计、数据回流管理（P36Admin，新增/联调完成）、数据回流明细（P36Detail，台账查看）、数据表管理（P37，管理数据表key与数据库表名映射） | 全平台数据 | 超管后台 → 开发者管理 / 能力管理 / 授权管理 / 审批管理 |
| 开发者管理员 | 开发者方管理员：企业信息维护、子账号管理、两步申请（先接口申请后数据授权，接口申请记录与数据授权记录均位于接口数据申请管理菜单下）、数据回流管理（P36，查看超管发起的数据回流记录，当对接方式为"开发者提供接口"时上传对接文档） | 本企业数据 | 开发者后台 → 接口管理 / 接口数据申请管理 / 数据回流 |
...
- **职责**：桂教通超管发起数据回流申请，选择对接方式（开发者提供接口 / 开发者主动推送）、推送方式（全量/增量）和目标开发者；开发者管理员在开发者端查看超管发起的记录，当对接方式为"开发者提供接口"时上传对接文档；超管确认联调完成。数据回流明细（P36Detail）台账式查看各开发者回流数据的明细
- **包含页面**：数据回流管理列表页（P36Admin，桂教通超管，含新增入口）、数据回流管理列表页（P36，开发者管理员，查看与上传文档）、新增/查看/上传对接文档页（P36Form）、数据回流明细页（P36Detail，桂教通超管，只读台账）、回流明细查看页（P36DetailView，桂教通超管，只读）
- **涉及角色**：桂教通超管（菜单：授权管理 → 数据回流管理 / 数据回流明细）、开发者管理员（菜单：数据回流 → 数据回流管理）
- **状态机**：对接方式为"开发者提供接口"：待对接（超管新增后）→ 联调中（开发者上传对接文档后）→ 已完成（超管联调完成）；对接方式为"开发者主动推送"：联调中（超管新增后）→ 已完成（超管联调完成）
- **业务流程**：见「五、核心业务流程 → 5.7 数据回流流程」

> P36Admin 列表（桂教通超管）：查询条件（名称、开发者名称、联调状态）；列表列（名称、应用名称、开发者名称、对接方式、推送方式、数据表、对接文档、联调状态、申请时间）；操作列（联调完成[联调中时展示]、查看）；**工具栏含「新增」按钮**，点击进入 P36Form 新增页面。对接文档列：对接方式为"开发者提供接口"且开发者已上传文档时展示文档列表，未上传时展示"待开发者上传"；对接方式为"开发者主动推送"时展示「—」。联调完成点击后二次确认，状态变为「已完成」。
> P36 列表（开发者管理员）：展示超管发起的与当前开发者相关的数据回流记录。查询条件（名称、联调状态）；列表列（名称、应用名称、对接方式、推送方式、对接文档、联调状态、申请时间）；操作列（上传文档[仅对接方式为"开发者提供接口"且状态为"待对接"时展示]、查看）；**无新增按钮**。对接文档列：同 P36Admin。
> P36Form 新增/查看/上传页：超管新增模式——名称（文本必填）、应用名称（下拉选择，根据应用名称模糊匹配，最多展示 30 个匹配应用，必填）、开发者（下拉选择，根据开发者名称模糊匹配，最多展示 30 个，必填）、对接方式（单选：开发者提供接口/开发者主动推送）、推送方式（单选：全量/增量，必填，两种对接方式均展示）。选"开发者提供接口"时展示：认证key（文本选填，加密存储）、对接域名（文本选填）；对接文档区域显示提示"由开发者上传"。选"开发者主动推送"时展示：唯一字段（文本必填，单个字段名，maxlength 64，位于推送方式与同步数据表之间）、同步数据表区域（多个数据表卡片，每个卡片含「数据表key」可搜索下拉[必填，数据来源数据表管理P37，仅展示启用状态的数据表]、「表 json」按钮[点击打开弹窗，弹窗内含 JSON 编辑器与输出预览]、「数据表文件」文件上传[选填]；可添加多个数据表卡片）与认证key（选填）。申请理由（多行文本，最多 500 字符）、免责声明（勾选同意）。提交后：对接方式为"开发者提供接口"→ 状态"待对接"；对接方式为"开发者主动推送"→ 状态"联调中"。开发者上传模式——所有字段只读，仅对接文档区域可编辑（上传附件），提交后状态从"待对接"变为"联调中"。查看模式只读展示。
> P36Detail 明细页（桂教通超管）：只读台账，按接口/数据表维度记录各开发者回流数据的明细（一个数据表记录一行）。查询条件（开发者名称、应用名称、回流状态）；列表列（序号、开发者名称、应用名称、数据表key、接口名称、同步数据量、回流状态、失败原因、最近同步时间）；操作列（手动同步、查看）。手动同步仅对接方式为"开发者提供接口"且回流状态为"失败"时展示；查看跳转 P36DetailView 只读页。回流明细查看页（P36DetailView）展示明细行详情：基本信息含推送方式、唯一字段；接口回流展示对接文档，数据表回流展示数据表key、表json（只读），操作仅「返回」。
...
- **职责**：桂教通超管管理业务场景的数据表key与数据库表名称映射，为数据回流（P36Form）中"开发者主动推送"对接方式提供数据表选择数据源；支持启用/停用状态管理，停用后不影响历史数据回流记录，但新增数据回流不可选择该数据表
- **包含页面**：数据表管理列表页（P37，含弹窗新增/编辑、启用/停用、删除）
- **涉及角色**：桂教通超管（菜单：授权管理 → 数据表管理）
- **关键规则**：数据表key全局唯一，不可重复；数据库表名称、业务场景说明必填；新增记录默认启用；启用状态或已被数据回流使用的数据表不可删除
...
桂教通超管在数据回流管理（P36Admin）点击「新增」，选择应用名称、目标开发者、对接方式（开发者提供接口 / 开发者主动推送）并提交：对接方式为"开发者提供接口"时填写认证key（选填）与对接域名（选填），提交后状态为「待对接」，对应开发者在开发者端数据回流管理（P36）可看到该记录并上传对接文档，提交后状态变为「联调中」，超管可查看开发者上传的对接文档；对接方式为"开发者主动推送"时填写同步数据表（数据表key + 表json）与认证key（选填），提交后状态直接为「联调中」，对应开发者可在P36查看数据。超管在联调中状态下点击「联调完成」二次确认后进入「已完成」。回流执行结果按数据表维度记录在数据回流明细（P36Detail）。

```mermaid
flowchart TD
    A[桂教通超管: P36Admin「新增」] --> B[选择应用 / 开发者 / 对接方式]
    B --> C{对接方式?}
...
| 页面编号 | 页面名称 | 所属模块 | 主要功能 | 适用角色 | 信息密度 |
|---------|---------|---------|---------|---------|---------|
| P36Admin | 数据回流管理列表（桂教通超管） | 数据回流管理 | 查询条件（名称/开发者名称/联调状态[待对接/联调中/已完成]）；列表展示（名称/应用名称/开发者名称/对接方式/推送方式/数据表[开发者提供接口时「接口对接」，开发者主动推送时展示数据表key]/对接文档[开发者提供接口且已上传时展示文档名点击预览，未上传时"待开发者上传"，开发者主动推送时「—」]/联调状态/申请时间）；操作列（联调完成[联调中时展示，点击二次确认→已完成]、查看）；**工具栏含「新增」按钮**，点击进入 P36Form 新增页面 | 桂教通超管 | 中 |
| P36 | 数据回流管理列表（开发者管理员） | 数据回流管理 | 展示超管发起的与当前开发者相关的数据回流记录；查询条件（名称/联调状态[待对接/联调中/已完成]）；列表展示（名称/应用名称/对接方式/推送方式/对接文档/联调状态/申请时间）；操作列（上传文档[仅对接方式为"开发者提供接口"且状态为"待对接"时展示，跳转 P36Form 上传模式]、查看）；**无新增按钮** | 开发者管理员 | 中 |
| P36Detail | 数据回流明细（桂教通超管） | 数据回流管理 | 只读台账，按接口/数据表维度记录各开发者回流数据的明细（一个数据表一行）；查询条件（开发者名称/应用名称/回流状态）；列表展示（序号/开发者名称/应用名称/数据表key/接口名称/同步数据量/回流状态[成功/失败标签]/失败原因/最近同步时间）；操作列（手动同步[对接方式为"开发者提供接口"且回流状态为失败时展示]、查看[跳转 P36DetailView]） | 桂教通超管 | 中 |
| P36DetailView | 回流明细查看（桂教通超管） | 数据回流管理 | 只读页，按明细行类别展示查看信息；基本信息（开发者名称/应用名称/回流类别[接口回流/数据表回流]/推送方式/唯一字段/同步数据量/回流状态/最近同步时间/失败原因）；类别内容：接口回流（对接文档列表预览），数据表回流（数据表key、表json只读展示）；操作仅「返回」 | 桂教通超管 | 低 |
| P36Form | 新增/查看/上传对接文档 | 数据回流管理 | 超管新增模式：名称（必填）、应用名称（下拉模糊匹配，必填）、开发者（下拉模糊匹配，必填）、对接方式（单选：开发者提供接口/开发者主动推送）、推送方式（单选：全量/增量，必填，两种对接方式均展示）。选"开发者提供接口"：认证key（选填，加密存储）、对接域名（选填）；对接文档由开发者后续上传。选"开发者主动推送"：唯一字段（文本必填，单个字段名，maxlength 64，位于推送方式与同步数据表之间）、同步数据表区域（数据表key[可搜索下拉，来源P37，仅展示启用状态的数据表]+表json[弹窗编辑]+数据表文件[选填上传]，可添加多个）、认证key（选填）。申请理由（必填，最多500字）、免责声明（勾选同意）。提交后：开发者提供接口→状态"待对接"，开发者主动推送→状态"联调中"。开发者上传模式：所有字段只读，仅对接文档区域可编辑上传，提交后状态"待对接"→"联调中"。查看模式全表单只读。 | 桂教通超管、开发者管理员 | 中 |

> 唯一字段+数据表交互：对接方式选"开发者主动推送"时，在推送方式下方、同步数据表区域上方展示「唯一字段」文本框（必填，单个字段名，maxlength 64）；同步数据表区域：每个数据表卡片含「数据表key」可搜索下拉（必填，数据来源数据表管理 P37，仅展示启用状态的数据表）、「表 json」按钮（点击打开弹窗，弹窗内含 JSON 编辑器[工具栏含「格式化」「清空」「复制结果」，支持 Text/Tree/Table 视图] + 输出预览面板）、「数据表文件」文件上传（选填）。
> 状态机：对接方式"开发者提供接口"：待对接 → 联调中（开发者上传文档后）→ 已完成（超管联调完成）；对接方式"开发者主动推送"：联调中 → 已完成（超管联调完成）。
> 列表数据表列：开发者提供接口时展示「接口对接」（对接文档独立成列）；开发者主动推送时展示数据表key（多个以英文逗号隔开）。

### 6.13 数据表管理模块

| 页面编号 | 页面名称 | 所属模块 | 主要功能 | 适用角色 | 信息密度 |
|---------|---------|---------|---------|---------|---------|
| P37 | 数据表管理 | 数据表管理 | 列表展示全部数据表记录（数据表key/数据库表名称/业务场景说明/状态[启用/停用，标签展示]/创建时间）；工具栏「新增」按钮打开弹窗新增（数据表key[必填，唯一]/数据库表名称[必填]/业务场景说明[必填]，新增默认启用）；操作列（编辑[弹窗编辑] / 启用停用[二次确认切换状态，停用时提示"停用后新增的数据回流将不可选择该数据表，已有数据不受影响"] / 删除[二次确认，启用状态或已被数据回流使用的数据表不可删除，按钮置灰]） | 桂教通超管 | 轻 |
...
| 字段 | 类型 | 长度 | 必填 | 默认值 | 枚举值 | 格式 | 业务来源 | 说明 |
|------|------|------|------|--------|--------|------|----------|------|
| 数据表key | string | 100 | 是 | — | — | — | 用户输入 | 数据表唯一标识，全局唯一不可重复 |
| 数据库表名称 | string | 200 | 是 | — | — | — | 用户输入 | 对应数据库中的实际表名 |
| 业务场景说明 | string | 500 | 是 | — | — | — | 用户输入 | 数据表的业务用途说明 |
| 状态 | string | 4 | 是 | 启用 | 启用、停用 | — | 系统记录 | 数据表是否可用；停用后不影响已有数据回流记录，但新增数据回流不可选择该数据表；启用状态或已被数据回流使用的数据表不可删除 |
| 创建时间 | datetime | — | 是 | 当前时间 | — | YYYY-MM-DD HH:mm | 系统生成 | 数据表记录创建时间 |
...
### 9.7 数据回流状态机（P36Admin/P36）

```text
对接方式 = "开发者提供接口"：
[待对接] ──开发者上传对接文档──→ [联调中] ──超管联调完成──→ [已完成]
...
- 状态：待对接、联调中、已完成
- 待对接 → 联调中：开发者上传对接文档并提交（仅对接方式为"开发者提供接口"时存在此状态）
- 联调中 → 已完成：超管「联调完成」（二次确认）
...
| R30 | 数据回流管理（P36Admin/P36/P36Form/P36Detail）：超管在 P36Admin 新增数据回流，选择目标开发者（可搜索下拉，单选）和对接方式（开发者提供接口/开发者主动推送）；对接方式为"开发者提供接口"时认证key选填、对接域名选填，对接文档由开发者后续上传，提交后状态为"待对接"；对接方式为"开发者主动推送"时填写同步数据表（数据表key+表json）与认证key（选填），提交后状态为"联调中"。开发者在 P36 查看超管分配的记录，当对接方式为"开发者提供接口"且状态为"待对接"时可上传对接文档并提交，提交后状态变为"联调中"。超管在 P36Admin 对"联调中"状态的记录点击「联调完成」二次确认后变为"已完成"。P36Admin 列表查询支持名称、开发者名称模糊匹配与联调状态（待对接/联调中/已完成）筛选；P36 列表查询支持名称与联调状态筛选。数据回流明细（P36Detail）为超管台账，按维度一行一条记录；操作列「手动同步」仅对接方式为"开发者提供接口"且回流状态为"失败"时展示 | 新增、上传文档、联调完成、明细查看 | 数据回流管理 |
...
| R36 | 数据表管理（P37）CRUD 与状态管理：数据表key全局唯一不可重复，数据库表名称与业务场景说明必填；新增记录默认启用；支持启用/停用切换（二次确认），停用时提示"停用后新增的数据回流将不可选择该数据表，已有数据不受影响"；启用状态或已被数据回流使用的数据表不可删除（删除按钮置灰）；仅停用且未被使用的数据表可删除（二次确认）；数据表key作为数据回流（P36Form）"开发者主动推送"对接方式中数据表选择的数据源，下拉仅展示启用状态的数据表 | 新增/编辑/启用停用/删除数据表 | 数据表管理 |
| R37 | 数据回流记录（P36Admin/P36/P36Form）新增推送方式字段（全量/增量），两种对接方式均必填；P36Admin与P36列表在对接方式后展示推送方式列；P36Form中对接方式为"开发者主动推送"时，唯一字段提升为表单级字段（必填，单个字段名，maxlength 64，位于推送方式与同步数据表之间），数据表key由文本输入改为可搜索下拉（数据来源P37），表json改为弹窗编辑、新增数据表文件（选填上传） | 新增/查看数据回流 | 数据回流管理 |
...
| 数据回流管理列表（P36Admin） | — | 全平台数据回流记录（含新增） | — |
| 数据回流管理列表（P36） | — | — | 本企业关联的数据回流记录 |
| 数据回流明细（P36Detail） | — | 全平台数据回流明细（只读台账） | — |
| 回流明细查看（P36DetailView） | — | 回流明细行查看（只读，按类别展示） | — |
| 新增/查看/上传对接文档（P36Form） | — | 新增数据回流 | 上传对接文档、查看 |
| 数据表管理（P37） | — | 全平台数据表（新增/编辑/删除） | — |
...
| 数据回流新增（超管） | — | ✅ | — | P36Admin 工具栏「新增」，跳转 P36Form 新增页，选择开发者（可搜索下拉）、对接方式；提交后根据对接方式设置初始状态 |
| 数据回流上传文档（开发者） | — | — | ✅ | P36 列表对接方式为"开发者提供接口"且状态"待对接"时可点击「上传文档」，跳转 P36Form 上传模式，提交后状态变为"联调中" |
| 数据回流联调完成（超管） | — | ✅ | — | "联调中"状态时展示，二次确认后状态变为"已完成" |
| 数据回流查看 | — | ✅ | ✅ | 跳转 P36Form 只读模式 |
| 数据回流明细查看（超管） | — | ✅ | — | 数据回流明细（P36Detail）台账页只读查看，支持按开发者名称/应用名称/回流状态筛选 |
| 数据回流手动同步（超管） | — | ✅ | — | 数据回流明细页操作列，仅对接方式为"开发者提供接口"且回流状态为"失败"时展示；二次确认弹窗后手动调接口拉取数据 |
| 新增数据表（超管） | — | ✅ | — | P37 弹窗新增：数据表key（唯一）、数据库表名称、业务场景说明均必填 |
| 编辑数据表（超管） | — | ✅ | — | P37 弹窗编辑：可修改数据库表名称与业务场景说明 |
| 删除数据表（超管） | — | ✅ | — | P37 删除需二次确认 |
