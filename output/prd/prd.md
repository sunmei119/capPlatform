...
5. 客户端授权下的接口授权记录与数据授权记录可逐条取消授权：列表操作列「取消授权」打开客户端取消授权页（P30AuthCancel，仅该客户端，界面同查看但不展示客户端信息区块），两个 Tab（接口授权记录/数据授权记录，机构申请仅接口）均含状态（启用/停用）列并分页展示；「启用」记录行内「取消授权」二次确认后仅把该记录置为「停用」（记录保留、数据仍在，停用期间该客户端不可调用对应接口 / 不可获取对应机构或学校的数据），停用记录行内「启用」可恢复；记录状态在「查看」（P30View）中只读同步展示；两个 Tab 均支持「全部取消 / 批量取消（勾选启用记录）」，二次确认后批量置停用。
...
1. 桂教通超管在数据回流管理（P36Admin）点击"新增"，选择应用名称、目标开发者（可搜索下拉）、对接方式（开发者提供接口/开发者主动推送）、推送方式（全量/增量）并填写相关信息提交。对接方式为"开发者提供接口"时填写认证key（选填）与对接域名（选填），提交后状态为"待对接"；对接方式为"开发者主动推送"时填写唯一字段（单个字段名，表单级别）与同步数据表（数据表key+表json）与认证key（选填），提交后状态为"联调中"。
...
flowchart TD
    ...
    F --> G[开发者上传对接文档并提交]
    G --> H[状态: 联调中]
    C -- 开发者主动推送 --> I[填写唯一字段 + 选择数据表key + 编辑表json]
    I --> J[提交, 状态: 联调中]
    J --> K[开发者管理员: P36 查看数据]
    H --> L[超管「联调完成」二次确认]
    K --> L
...
- **推送方式**：单选（全量/增量），必填；两种对接方式下均需选择。
  ...
  - 选「开发者主动推送」：先展示「唯一字段」（文本输入，必填，最多 64 字符，仅允许填写单个字段名；位于推送方式与同步数据表之间，属于表单级别字段，不在数据表卡片内），再展示「同步数据表」区域，包含多个数据表卡片，每个卡片含「数据表key」（下拉选择，可搜索，数据来源 P37 数据表管理中启用状态的记录，必填）、「表json」（点击「编辑表json」按钮打开弹窗编辑，弹窗内含 JSON 编辑器 + 输出预览 + 校验，提交时校验为合法 JSON 对象，必填）与「数据表文件」（文件上传，选填，后端根据文件生成数据表）；每个卡片可删除（至少保留一个）；底部「+ 添加数据表key」按钮新增卡片。同时展示认证key（选填）。
- **申请理由**：多行文本，必填，最多 500 字符，显示字数统计。
- **免责声明**：「我已阅读并同意」+ 可点击的《数据使用免责申明》链接，弹窗展示声明全文，弹窗内也可一键勾选同意。
- **提交校验**：名称必填；应用名称必选；开发者名称必选；推送方式必选；对接方式为「开发者主动推送」时唯一字段必填（最多 64 字符，仅允许单个字段名）、每个数据表卡片必须选择数据表key、且表json 为合法 JSON 对象；对接域名若存在须以 http:// 或 https:// 开头；申请理由必填；免责声明必须勾选。
...
- **查看**：点击打开「客户端信息查看」新界面（P30View，非弹窗），以表单文本只读展示该客户端申请/维护的全部信息（客户端ID、客户端密钥、客户端类型、回调地址、登出后回调地址[仅平台授权展示]、访问令牌有效期、刷新令牌有效期、白名单、备注、启用状态、授权范围），样式同申请记录查看（P10）。其中「客户端秘钥」默认脱敏展示（保留前4位与后4位，中间以 `****` 代替），以蓝色字体显示，右侧小眼睛图标点击可在「脱敏/明文」间切换，点击秘钥串可复制完整秘钥。若已维护授权信息，界面内以 **Tab 页**形式分「接口授权」与「数据授权」两个页签展示（默认选中「接口授权」Tab）：
  - 「接口授权」Tab（接口名称 / 最大调用量/日（次/日） / 接口平台 / 数据敏感等级 / 查看文档[点击跳转接口文档查看页 P06] / 状态），一条记录 = 该客户端当前授权的一个接口，多个接口拆多行（是否全部接口=是时按平台可见接口逐条展开）。
  - 「数据授权」Tab（同步范围类型 / 学校名称 / 机构名称 / 授权截止时间 / 状态）：一条记录 = 一个学校/教育局（一次录入多个单位按单位拆分，「是否全部数据=是」时为一条「全部数据」全区记录）；仅客户端类型为「平台授权」时展示该 Tab；客户端类型为「机构申请」时不展示。
  - 两个 Tab 的授权记录均含「状态（启用/停用）」列（只读展示，停用由取消授权产生、记录保留，本页无操作），均支持分页（每页 5 条，底部展示总数与翻页）。
  - 若未维护授权信息，则不展示 Tab 区块。
- **取消授权**：点击打开「客户端取消授权」新界面（P30AuthCancel，非弹窗，仅当前客户端，界面同查看的授权记录展示但不展示客户端信息区块），在该页按接口/数据单位逐条查看接口授权记录与数据授权记录并「取消授权 / 启用」，详见本节（五）。
...
由 P30 列表操作列「取消授权」进入的新开界面（非弹窗，仅当前客户端），页面主体同「查看」（P30View）的授权记录展示（分「接口授权记录」「数据授权记录」两个 Tab，默认选中「接口授权记录」），**但不展示客户端信息区块**；页顶提示条展示当前客户端ID与操作说明：「取消授权」将对应授权置为「停用」（记录保留、数据仍在），停用期间该客户端将不可再调用对应接口 / 获取对应机构或学校的数据；可随时重新「启用」。返回回到 P30。

**（1）接口授权记录 Tab**

- 列表：该客户端当前授权的一个接口为一条记录（接口名称 / 最大调用量/日（次/日） / 接口平台 / 数据敏感等级 / 查看文档[蓝色链接，点击跳转接口文档查看页 P06] / 状态[启用/停用]）。若接口授权「是否全部接口=是」，按平台当前可见接口逐条展开；分页（每页 5 条）。无记录时展示空态「暂无接口授权记录」。
- 行内操作：状态为「启用」的记录展示「取消授权」，点击弹窗二次确认「确认取消该接口的授权？取消后该接口授权将停用（记录保留、数据仍在），停用期间客户端将不可调用对应接口。」，确认后仅把该记录状态置为「停用」（记录保留、数据仍在，仍展示在列表）；状态为「停用」的记录展示「启用」，二次确认「确认启用该接口的授权？启用后客户端可调用对应接口。」后恢复为「启用」。
- 批量取消：列表行前提供多选框（仅状态为「启用」的记录可选），工具栏提供「全部取消」与「批量取消（已选 N 条）」按钮（无启用记录或未勾选时置灰）。点「全部取消」弹窗二次确认「确认全部取消该客户端的接口授权？共 N 条接口授权记录将被置为停用（记录保留、数据仍在），停用期间客户端将不可调用这些接口。」，确认后该 Tab 当前全部启用记录统一置为「停用」；勾选若干启用记录后点「批量取消」弹窗二次确认「确认取消选中的 N 条接口授权？取消后将被置为停用（记录保留、数据仍在），停用期间客户端将不可调用对应接口。」，确认后所选记录统一置为「停用」；取消后记录仍展示、状态为停用，可单独或再次批量重新「启用」。

**（2）数据授权记录 Tab**

- 仅客户端类型为「平台授权」时展示该 Tab；客户端类型为「机构申请」时不展示（无数据授权）。
- 列表：该客户端当前授权的一个数据单位（学校/教育局）为一条记录（同步范围类型 / 学校名称 / 机构名称 / 授权截止时间 / 状态[启用/停用]）。一次录入多个单位按单位拆分（一个学校/教育局一条）；「是否全部数据=是」时为一条「全部数据」全区记录；分页（每页 5 条）。无记录时展示空态「暂无数据授权记录」。
- 行内操作：状态为「启用」的记录展示「取消授权」，点击弹窗二次确认「确认取消该数据授权？取消后该授权将停用（记录保留、数据仍在），停用期间客户端将不可获取对应机构或学校的数据。」，确认后仅把该记录状态置为「停用」（记录保留、数据仍在）；停用记录展示「启用」，二次确认后恢复为「启用」。
- 批量取消：列表行前提供多选框（仅状态为「启用」的记录可选），工具栏提供「全部取消」与「批量取消（已选 N 条）」按钮（无启用记录或未勾选时置灰）。点「全部取消」弹窗二次确认「确认全部取消该客户端的数据授权？共 N 条数据授权记录将被置为停用（记录保留、数据仍在），停用期间客户端将不可获取对应机构或学校的数据。」，确认后该 Tab 当前全部启用记录统一置为「停用」；勾选若干启用记录后点「批量取消」弹窗二次确认「确认取消选中的 N 条数据授权？取消后将被置为停用（记录保留、数据仍在），停用期间客户端将不可获取对应机构或学校的数据。」，确认后所选记录统一置为「停用」；取消后记录仍展示、状态为停用，可单独或再次批量重新「启用」。
...
| 客户端信息查看（P30View） | — | 全平台客户端（只读，授权记录含状态列） | — |
...
| 客户端授权信息查看 | — | ✅ | — | P30 列表操作列「查看」打开 P30View 新界面只读展示（接口/数据授权记录含状态列） |
| 客户端授权启用/停用 | — | ✅ | — | 需二次确认；停用后该客户端不可再调用授权接口 |
| 取消客户端接口授权 | — | ✅ | — | P30 列表操作列「取消授权」→ P30AuthCancel 接口授权记录 Tab，状态为「启用」的记录行内「取消授权」，二次确认「取消后客户端将不可调用对应接口」后仅把该记录状态置为停用（记录保留、数据仍在，可重新启用） |
| 取消客户端数据授权 | — | ✅ | — | P30 列表操作列「取消授权」→ P30AuthCancel 数据授权记录 Tab，状态为「启用」的记录行内「取消授权」，二次确认「取消后客户端将不可获取对应机构或学校的数据」后仅把该记录状态置为停用（记录保留、数据仍在，可重新启用） |
...
> 客户端授权下的接口授权记录与数据授权记录（非独立新增流程，由 P31/P31B 授权配置派生并按单位拆分展示）：接口授权记录一条=一个已授权接口（「是否全部接口=是」时按平台当前可见接口逐条展开）；数据授权记录一条=一个学校/教育局（一次录入多个单位按单位拆分，「是否全部数据=是」时为一条「全部数据」全区记录）。每条授权记录具有「状态（启用/停用）」：取消授权（P30AuthCancel）二次确认后将对应记录置为停用（记录保留、数据仍在，可重新启用）；记录停用与整条客户端「使用状态」相互独立——记录停用仅限制对应接口调用或对应机构/学校数据获取。
...
- 停用 → 启用：启用后恢复调用权限。
- 客户端授权下的接口授权记录与数据授权记录同样具有「启用/停用」状态：启用 → 停用（取消授权，在 P30AuthCancel 操作列逐条二次确认，记录保留、数据仍在，可重新启用）；停用 → 启用（「启用」二次确认后恢复）。记录停用与整条客户端的启用/停用相互独立；记录状态在 P30View 只读展示。
...
| R38 | 取消客户端接口授权：P30 列表操作列「取消授权」打开客户端取消授权页（P30AuthCancel）→ 接口授权记录 Tab，状态为「启用」的记录行内「取消授权」，点击二次确认后仅把该接口授权记录状态置为「停用」（记录保留、数据仍在），停用期间客户端将不可调用对应接口；停用记录行内「启用」可恢复 | 取消授权（客户端接口授权记录） | 客户端授权管理 |
| R39 | 取消客户端数据授权：P30 列表操作列「取消授权」→ P30AuthCancel 数据授权记录 Tab，状态为「启用」的记录行内「取消授权」，二次确认后仅把该记录状态置为「停用」（记录保留、数据仍在），停用期间客户端将不可获取对应机构或学校的数据；停用记录行内「启用」可恢复；机构申请类型客户端不展示数据授权 Tab | 取消授权（客户端数据授权记录） | 客户端授权管理 |
| R40 | 客户端接口授权记录与数据授权记录的状态（启用/停用）在查看（P30View）与取消授权（P30AuthCancel）两页对应 Tab 均展示「状态」列；P30View 为只读展示（无操作列），P30AuthCancel 提供行内「取消授权/启用」操作 | 授权记录展示与操作 | 客户端授权管理 |
| R41 | 客户端取消授权（P30AuthCancel）接口授权记录与数据授权记录两个 Tab 均支持批量取消：行前多选框（仅「启用」记录可选）+ 工具栏「全部取消」（二次确认提示共 N 条，取消该 Tab 当前全部启用记录）与「批量取消（已选 N 条）」（勾选后二次确认取消所选记录）；取消将对应启用记录统一置为「停用」（记录保留、数据仍在，可重新启用）；无可取消记录或未勾选时按钮置灰 | 批量取消（客户端接口/数据授权记录） | 客户端授权管理 |
...
- AC-CLT-05：P30 列表操作列新增「取消授权」（平台授权与机构申请类型均展示）打开客户端取消授权页（P30AuthCancel，仅当前客户端）：界面同查看（P30View）的授权记录展示但不展示客户端信息区块；「接口授权记录」「数据授权记录」两 Tab 均含状态（启用/停用）列并分页（每页 5 条，机构申请类型仅展示接口授权记录 Tab）；状态为「启用」的记录行内展示「取消授权」，点击二次确认（接口授权记录提示「取消后客户端将不可调用对应接口」、数据授权记录提示「取消后客户端将不可获取对应机构或学校的数据」）后仅把该记录状态置为「停用」（记录保留、数据仍在），记录仍保留展示；停用记录行内展示「启用」，二次确认后恢复为启用。
- AC-CLT-06：客户端信息查看（P30View）的「接口授权」「数据授权」Tab 均含「状态（启用/停用）」列只读展示（无操作列），反映各授权记录的取消授权（停用）/启用状态。
- AC-CLT-07：客户端取消授权（P30AuthCancel）的「接口授权记录」「数据授权记录」两个 Tab 均支持批量取消：行前多选框仅状态为「启用」的记录可选，工具栏提供「全部取消」与「批量取消（已选 N 条）」（无启用记录或未勾选时置灰）；点「全部取消」弹窗二次确认（提示共 N 条将被置为停用）后，该 Tab 当前全部启用记录统一置为「停用」（记录保留、数据仍在）；勾选若干启用记录后点「批量取消」二次确认（提示所选条数），所选记录统一置为「停用」；取消后记录仍保留展示、状态为停用，可单独或再次批量重新「启用」。

### （十一）数据回流（P36/P36Admin/P36Detail）

- AC-RFL-01：超管在 P36Admin 点击「新增」跳转 P36Form 新增模式；开发者名称为可搜索下拉（查询平台所有开发者，单选）；对接方式单选（开发者提供接口/开发者主动推送）控制展示区域；推送方式（全量/增量）为必填单选，两种对接方式下均需选择；名称必填、推送方式必选、申请理由必填（最多 500 字符）、免责声明必须勾选；对接方式为「开发者主动推送」时唯一字段必填（文本输入，最多 64 字符，仅允许单个字段名，表单级别字段，位于推送方式与同步数据表之间）、每个数据表卡片必须选择数据表key（下拉选择，数据来源 P37 数据表管理中启用状态的记录）、且表json 为合法 JSON 对象（弹窗方式编辑），可上传数据表文件（选填）。
...
- AC-TBL-02：新增弹窗填写数据表key（必填，全局唯一）、数据库表名称（必填）、业务场景说明（选填）；数据表key 重复时提示「数据表key 已存在」并阻止提交；新增记录默认状态为「启用」。
- AC-TBL-03：编辑弹窗回显当前值，数据表key 只读不可修改，数据库表名称与业务场景说明可编辑。
...
- AC-TBL-06：P37 启用状态的数据表记录作为 P36Form 中「数据表key」下拉选择的数据来源；停用的数据表不出现在下拉选项中；P37 新增/编辑/删除/启用/停用后，P36Form 下拉选项实时更新。
