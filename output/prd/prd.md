...
3. 客户端授权管理中存在「机构申请」类型的记录（由机构组织同步申请经「审批管理 → 组织同步审批（P41）」审批通过后自动生成，含客户端ID与客户端秘钥），本列表不提供审批操作，操作列统一为启用/查看/接口授权。
4. 对已有记录可查看、编辑或删除；点击"数据授权"跳转数据授权配置页（P31）维护数据授权信息（是否全部数据/同步范围类型/学校名称或机构名称/授权截止时间），点击"接口授权"跳转接口授权配置页（P31B）维护接口授权信息（是否全部接口/授权接口）；数据授权与接口授权分开维护。启用状态记录数据的停用/启用状态（启用 / 停用）。
...
- 存量指标（取当前实时值，不随所选时间范围变化）：接口总数、开发者总数、已授权应用数、接口分类分布、应用调用总次数、应用调用成功率。
- 期间指标（统计所选时间范围内新增/产生的数据量）：接口申请量、接口授权量、接口申请量 TOP5 榜单——「今日」取当日数据，「近7天 / 近30天」取区间累计数据，「自定义」取所选区间累计数据。
- 接口调用趋势：统计接口申请量 TOP5 接口在所选时间范围内的成功/失败调用量——「今日」取当日调用量，「近7天 / 近30天」取区间累计调用量，「自定义」取所选区间累计调用量。
- 应用维度统计（应用调用趋势 / 应用调用量 TOP5 / 应用授权接口数分布）：以"应用"为聚合维度统计调用量 TOP5 应用的成功/失败调用量、调用量榜单、各应用授权接口数；时间范围口径与接口维度一致。

**维度统计说明**（看板底部"维度统计说明"区同步展示，解释各统计维度聚合口径）：

- 接口维度：面向"单个接口"聚合，反映接口运行与使用情况。覆盖接口总数（含敏感/非敏感）、接口分类分布（基础服务/业务服务/数据服务）、接口申请量 TOP5（按调用量排序）、接口调用趋势（TOP5 接口成功/失败调用量）；用于识别高频与异常接口。
- 开发者维度：面向"开发者（企业/个人）"聚合，反映供需方规模与活跃度。覆盖开发者总数（含未合作/合作中/已取消合作状态分布）、接口授权量（被授权应用数）；用于掌握生态开发者结构。
- 应用维度：面向"授权应用"聚合，反映各应用调用表现。覆盖已授权应用数、应用调用总次数、应用调用成功率，以及应用调用趋势、应用调用量 TOP5、应用授权接口数分布；用于识别高活跃应用。
...
- **接入凭证**：仅当状态为"审批通过 / 已完成"时展示；包含应用ID（appId）与 suiteKey。
- **审核信息**：审批结果（审批通过/已驳回/待审批标签）、审批意见、审批时间、**联调完成时间**（未完成时显示占位提示）。
...
本页面由桂教通超管在「能力管理 → 应用管理」打开，查看并管理**所有厂商的应用**，列表展示内容同开发者应用管理（P28）但**不展示「状态」列**，另增「开发者名称」列。应用的新增、编辑由对应开发者在其应用管理（P28）中完成（本页无「新增应用」入口）；桂教通超管可对任意厂商应用直接执行接口授权、数据授权与取消授权，产生的授权记录在本模块独立维护，与应用申请审批、客户端授权（P30）等流程相互独立（原授权管理下的独立数据授权管理入口已移除）。
...
| 授权接口 | 弹窗多选，可按接口名称/接口服务名搜索；候选为平台开放（对外可见）且「是否需要授权=是」的接口；已选接口以标签展示，可逐个移除 |
...
- **接口授权记录 Tab**：展示该应用当前已授权的接口（接口名称 / 接口服务名 / 数据敏感等级 / 授权截止时间 / 状态），每条 = 一个接口授权。点「取消授权」二次确认「取消后厂商将不可调用对应接口」，确认后**仅把该记录状态置为「停用」——记录保留、数据仍在**（应用不可调用对应接口）；停用记录点「启用」二次确认后可恢复为启用。
...
- **接口授权记录 Tab**：接口名称 / 接口服务名 / 数据敏感等级 / 授权截止时间 / 状态，分页（每页 5 条）。
...
只读展示全部字段（含应用名称、开发者名称、对接方式、推送方式）。对接方式为「开发者提供接口」时只读展示对接文档（有则展示可预览，无则展示「待开发者上传」）与认证key（已加密展示为「••••••（已加密）」）、对接域名；对接方式为「开发者主动推送」时只读展示表单级别的「唯一字段」，并逐卡片只读展示「数据表key」及「表json」完整内容。查看时免责声明展示「已同意免责声明」。
...
### （十）客户端授权管理（桂教通超管，P30/P31）

本模块由桂教通超管维护 OAuth 客户端与平台的授权关联关系，对应菜单：桂教通超管后台 → 授权管理 → 客户端授权管理。包含客户端授权管理列表（P30）、新增/编辑客户端授权（P31）与数据授权配置（P31）、接口授权配置（P31B）三个页面；数据授权与接口授权分开维护，分别由列表操作列「数据授权」「接口授权」打开新开表单页。客户端授权管理列表不提供「申请」「审批」操作入口：平台授权类客户端由工具栏「新增」创建，机构申请类客户端由组织同步审批（P41）通过后自动生成。
...
| 客户端类型=机构申请操作 | 启用 / 查看 / 接口授权 |
| 客户端类型=平台授权操作 | 编辑 / 启用停用 / 数据授权 / 接口授权 / 查看 |
...
- **数据授权**：点击跳转数据授权配置页（P31，新开表单页），展示并维护该客户端的数据授权信息（客户端ID/密钥只读；该页只维护数据授权，不维护接口授权）。页面字段：
  ...
  - **授权截止时间**（时间控件，必填）：不可选择当前时间之前的时间，只能选择当前时间的三年内的时间。
  - 已删除「是否包含下级」。保存后更新列表「授权范围」字段（仅数据授权部分）。
- **接口授权**：点击跳转接口授权配置页（P31B，新开表单页），展示并维护该客户端的接口授权信息（客户端ID/密钥只读；该页只维护接口授权，不维护数据授权）。页面字段：
  - **是否全部接口**（单选，必填：是 / 否）：选择「是」时授权的是全部接口，不展示授权接口；选择「否」时展示授权接口。
  - **授权接口**（是否全部接口=否时展示，必填）：弹窗接口列表多选，已选接口以标签展示可逐个移除。
  - 已删除「是否包含下级」。
- **查看**：点击打开「客户端信息查看」新界面（P30View，非弹窗），以表单文本只读展示该客户端申请/维护的全部信息（客户端ID、客户端密钥、客户端类型、回调地址、登出后回调地址[仅平台授权展示]、访问令牌有效期、刷新令牌有效期、白名单、备注、启用状态、授权范围），样式同申请记录查看（P10）。其中「客户端秘钥」默认脱敏展示（保留前4位与后4位，中间以 `****` 代替），以蓝色字体显示，右侧小眼睛图标点击可在「脱敏/明文」间切换，点击秘钥串可复制完整秘钥。若已维护授权信息，界面内额外以列表形式展示「数据授权」与「接口授权」：
  - 「数据授权」列表（同步范围类型 / 学校名称 / 机构名称 / 授权截止时间，多个机构拆多行）：仅客户端类型为「平台授权」时展示；客户端类型为「机构申请」时不展示该区块。
  - 「接口授权」列表（接口名称 / 接口平台 / 数据敏感等级 / 查看文档[点击跳转接口文档查看页 P06]），多个接口拆多行。
  - 「数据授权」与「接口授权」两个列表均支持分页（每页 5 条，底部展示总数与翻页）。
  - 若未维护授权信息，则不展示上述两个列表。
...
由列表操作列「数据授权」跳转的新开表单页，客户端ID 与客户端密钥以只读展示，仅维护该客户端的数据授权信息（接口授权由 P31B 单独维护，本页不涉及；已删除「是否包含下级」）。
...
由列表操作列「接口授权」跳转的新开表单页，客户端ID 与客户端密钥以只读展示，仅维护该客户端的接口授权信息（数据授权由 P31 单独维护，本页不涉及；已删除「是否包含下级」）。
...
| 应用接口授权（超管） | — | ✅ | — | 应用管理（P28All）操作列「接口授权」跳 P28ApiAuth：授权接口候选为平台开放（对外可见）且「是否需要授权=是」的接口，保存即生效（无审批），写应用级接口授权记录 |
...
| 学校名称 | string | 500 | 是（同步范围类型含"学校"时必填） | — | — | 多个学校用英文分号（;）隔开 | 用户输入（多行文本） | 数据授权申请（P28Apply）勾选「学校」时展示；同步覆盖的学校名称，按名称精确匹配其机构管理员，如：南宁市教育局;南宁市第三中学；客户端授权配置（P31）是否全部数据=否且勾选学校时、应用数据授权（P28DataAuth）勾选学校时同样展示 |
| 机构名称 | string | 500 | 是（同步范围类型含"教育局"时必填） | — | — | 多个机构用英文分号（;）隔开 | 用户输入（多行文本） | 数据授权申请（P28Apply）勾选「教育局」时展示；同步覆盖的机构名称，按名称精确匹配其机构管理员，如：南宁市教育局;桂林市教育局；客户端授权配置（P31）是否全部数据=否且勾选教育局时、应用数据授权（P28DataAuth）勾选教育局时同样展示 |
| 授权截止时间 | date | — | 是 | — | — | YYYY-MM-DD（时间控件） | 用户选择 | 授权到期时间；接口申请（P28ApplyApi）、数据授权申请（P28Apply）、客户端授权配置（P31）、应用接口授权（P28ApiAuth）、应用数据授权（P28DataAuth）均必填；不可选择今天之前的时间，最长不超过当前时间三年 |
...
| 是否全部数据 | string | 2 | 否 | 否 | 是、否 | — | 用户单选 | P31 授权配置 |
...
| R33 | 应用接口授权（P28ApiAuth）：授权接口候选为平台开放（对外可见）且「是否需要授权=是」的接口，保存即生效（无审批），按应用写应用级接口授权记录 | 接口授权保存 | 超管应用管理 |
...
- AC-AP-06：接口授权（P28ApiAuth）：应用名称与所属厂商只读；授权接口候选为平台开放（对外可见）且「是否需要授权=是」的接口（可按名称/服务名搜索多选）；授权截止时间必填（不可选今天之前、只能三年内）；至少选择一项接口方可保存；保存即生效（无审批）并在取消授权-接口授权记录 Tab 可见。
...
- AC-CLT-03：数据授权配置（P31 新开表单页）与接口授权配置（P31B 新开表单页）分别由 P30 操作列「数据授权」「接口授权」跳转，分开维护；两页客户端ID 与客户端密钥均只读展示；数据授权页是否全部数据选「是」时不展示同步范围类型、学校名称、机构名称，授权截止时间不可选择今天之前、只能选择三年内；接口授权页是否全部接口选「是」时不展示授权接口。
