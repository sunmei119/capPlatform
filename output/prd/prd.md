...
- 操作列含四个动作（每个动作均针对当前行应用）：
  - **查看**：打开 P29 查看模式，以文本只读形式展示应用基础信息、接入凭证（仅审批通过后展示应用ID与 suiteKey）及审核信息。
...
- **保存后**：按应用写接口授权记录，提示「接口授权已保存，厂商可调用所选接口」，返回应用管理（P28All）；该应用的已授权接口可在取消授权-接口记录 Tab 查看并取消。
...
- **接口记录 Tab**：展示该应用当前已授权的接口（接口名称 / 接口服务名 / 数据敏感等级 / 授权截止时间 / 状态），每条 = 一个接口授权。点「取消授权」二次确认「取消后厂商将不可调用对应接口」，确认后**仅把该记录状态置为「停用」——记录保留、数据仍在**（应用不可调用对应接口）；停用记录点「启用」二次确认后可恢复为启用。
...
### （九）数据回流管理（P36/P36Form/P36Admin/P36Detail）
...
| 取消应用接口授权（超管） | — | ✅ | — | 应用管理（P28All）操作列「取消授权」→ 接口记录 Tab，二次确认「取消后厂商将不可调用对应接口」后将记录状态置为停用（记录保留，可重新启用） |
...
| R35 | 取消接口授权需二次确认；取消即把该接口授权状态置为「停用」（记录保留、数据仍在），停用期间厂商将不可调用对应接口；停用记录可重新「启用」恢复 | 取消授权（接口记录） | 超管应用管理 |
...
- AC-AP-06：接口授权（P28ApiAuth）：应用名称与所属厂商只读；授权接口候选为平台开放（对外可见）且「是否需要授权=是」的接口（可按名称/服务名搜索多选）；授权截止时间必填（不可选今天之前、只能三年内）；至少选择一项接口方可保存；保存即生效（无审批）并在取消授权-接口记录 Tab 可见。
...
- AC-AP-08：取消授权（P28AuthCancel）：两 Tab（接口记录/数据授权记录）均含「状态（启用/停用）」列并分页展示该应用记录；状态为「启用」的记录行内展示「取消授权」，点击二次确认（接口记录提示「取消后厂商将不可调用对应接口」、数据授权记录提示「取消后厂商将不可获取对应机构或学校的数据」）后仅把该记录状态置为「停用」，记录仍保留展示；停用记录行内展示「启用」，二次确认后恢复为启用。
