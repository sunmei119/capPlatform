...
- 桂教通超管：能力管理（→开发者管理 P01、API接口管理 P04、接口能力管理 P35、接口分类管理 P04a、接口申请记录 P08、接口调用记录 P27、运营统计 P13）、授权管理（→客户端授权管理 P30、数据授权管理 P34、数据同步审批 P33）
- 开发者管理员：账号管理（→P16/P17）、接口中心（→P19）、应用管理（→P28、接口申请记录 P08、数据授权记录 P08Auth）
...
开发者入驻与合作流程分为邀请注册、联系跟进、确认合作、取消合作、重新合作五个阶段，由平台侧（桂教通超管）与开发者方协作完成（取消合作入口仅提供于开发者列表页，开发者详情页不提供该按钮）。

1. 平台侧新增开发者，或通过"邀请开发者"向开发者管理员发送邀请链接；开发者管理员收到链接后填写开发者信息并提交注册。
2. 平台侧确认联系，记录联系状态为已联系；确认联系不改变合作状态。
3. 确认合作时填写合作原因与合作时间；确认合作后开发者管理员获得开发者平台登录权限（租户体系复用桂教通）。
4. 取消合作时填写取消合作原因与时间；取消后开发者管理员手机号不可再登录开发者平台（入口在开发者列表页）。
5. 已取消的开发者可重新合作，重新合作后合作状态回到合作中，并恢复开发者管理员登录权限。
...
5. 开发者管理员在应用管理（P28）操作列点击"授权申请"，打开数据授权申请页（P28Apply），填写授权接口、最大调用量/日、同步范围类型、机构名称、是否包含下级、申请理由并同意免责声明后提交。
...
flowchart TD
    ...
    B --> C[接口申请页 P28ApplyApi: 应用名称 + 授权接口]
    ...
    H --> I[数据授权申请页 P28Apply: 接口/最大调用量/数据范围/机构/理由]
...
桂教通超管为应用配置数据授权（应用、同步范围类型、机构、授权接口），并管理启用/停用。

1. 桂教通超管在授权管理 → 数据授权管理（P34）列表查询数据授权记录（按应用名称、状态筛选）。
2. 点击"新增数据授权"，弹窗填写应用名称（可搜索下拉框，按应用名称模糊匹配，单选，必填，最多匹配 30 个）、同步范围类型（必填，可多选，枚举学校/教育局）、机构名称（必填，多个机构用英文分号(;)隔开，如：南宁市教育局;南宁市第三中学）、授权接口（必填，弹窗接口列表选择，可多选，支持按接口名称搜索）。
3. 保存后建立数据授权记录；对已有记录可编辑（修改应用/范围/机构/接口）或启用/停用（二次确认）。停用后应用不可再调用被授权接口。

```mermaid
flowchart TD
    A[桂教通超管] --> B[数据授权管理列表 P34 查询]
    B --> C{操作}
    C -- 新增 --> D[填写应用/同步范围类型/机构名称/授权接口]
    D --> E[建立数据授权记录]
    C -- 编辑 --> F[修改授权范围/接口]
    C -- 启用/停用 --> G[二次确认后切换状态]
...
本模块覆盖开发者从入驻邀请到合作的全生命周期，含开发者信息维护、联系跟进与合作状态变更，对应设计基线模块 4.1（开发者管理）。包含开发者列表（P01）、新增/编辑开发者（P02）、开发者详情（P03）三个页面。桂教通超管可见全平台开发者。

#### 开发者列表（P01）

页面由查询区、结果列表与行操作区组成。用户进入页面后应先能判断当前有哪些开发者、各自处于什么合作阶段，并直接发起新增、邀请或对单条开发者的联系、合作操作。

（1）查询开发者

列表展示当前筛选条件下的开发者记录，供用户快速判断开发者身份与合作状态。列表列包含：开发者名称、开发者类型、管理员姓名、手机号码、是否已联系、合作状态、是否在门户展示、申请日期。
...
点击"邀请开发者"弹出邀请成功弹窗，弹窗内容为蓝色对勾图标 + 提示文字"邀请链接已复制，可将此链接发给开发商进行注册"，底部两个按钮：

- "我知道了"：关闭弹窗。
- "查看链接"：在新浏览器页签中打开新增开发者页（P02），该页面不显示头部导航栏和侧栏菜单（standalone 模式），开发者管理员收到链接后填写开发者信息完成注册。注册成功后开发者出现在列表中，合作状态为"未合作"、申请日期为注册完成时间。
...
点击行尾"编辑"跳转编辑开发者页。仅"是否已联系=否"的开发者可编辑；已确认联系的开发者行不显示"编辑"按钮，只能查看。

（6）确认联系

点击行尾"确认联系"弹出确认框，确认后该开发者"是否已联系"置为"是"，合作状态保持不变。

（7）确认合作

点击行尾"确认合作"弹出确认框，填写合作原因与合作时间后确认。仅"是否已联系=是"的开发者可执行确认合作，未确认联系的开发者该按钮不可点击。

- 确认合作成功后，开发者管理员获得开发者平台登录权限（租户体系复用桂教通）；合作状态变为"合作中"。
...
页面为开发者信息表单，用于新建开发者或维护未确认联系开发者的信息。整体分为三个可折叠区：基础信息（必填集中）、财务信息（选填）、开票信息（选填）。页面底部提供保存与返回按钮。
...
- 保存成功后跳转开发者列表，新开发者合作状态为"未合作"、申请日期为当前时间。
- 保存失败（如网络异常）时保留已填写内容，提示"保存失败，请重试"。

（2）编辑开发者

仅"是否已联系=否"的开发者可进入编辑。表单结构与新增一致并回显当前值，保存规则同新增；已确认联系的开发者不可再编辑，只能通过开发者详情页 P03 查看。

（2）编辑开发者

仅"是否已联系=否"的开发者可进入编辑。表单字段与新增一致并回显当前值，保存规则同新增；已确认联系的开发者不可再编辑。

#### 开发者详情（P03）

页面以只读表单形式展示开发者完整信息，分为两个可折叠区（开发者基础信息、合作信息），并承载联系、合作、重新合作状态变更操作（取消合作入口在开发者列表页，详情页不提供该按钮）。
...
点击"确认联系"弹出确认框，确认后"是否已联系"置为"是"，"联系时间"记录当前时间。仅未联系开发者可操作。

（3）确认合作

点击"确认合作"弹出确认框，填写合作原因（必填）、合作时间（必填）后确认。仅"是否已联系=是"的开发者可操作。
...
右侧展示当前选中能力已关联的接口，列包含：接口名称、接口服务名称、数据敏感等级、接口路径。列表顶部展示关联数量与「添加接口」按钮，操作列提供「移出」。
...
列表展示当前筛选条件下的接口申请记录，列包含：应用名称、开发者名称（桂教通超管可见）、授权接口、申请时间、审批状态。支持按接口名称、审批状态筛选。
...
页面由查询区、结果列表与行操作区组成。记录开发者提交的数据授权申请（含数据范围、机构、最大调用量等），由机构管理员审批；审批通过后开发者获取数据授权。

（1）查询申请记录

列表展示当前筛选条件下的数据授权记录，列包含：应用名称、同步数据范围、机构名称、申请理由、申请时间、审批状态。支持按接口名称、应用ID、审批状态筛选。
...
- 申请信息区块以只读表单展示：应用名称、开发者名称、申请时间、审批状态。
- 申请接口列表位于申请信息下方，列包含：接口名称、接口服务名、接口能力、数据敏感等级、接口分类、**最大调用量/日（可编辑，正整数，必填）**；列表分页展示，每页默认 5 条，可翻页查看全部接口。
...
- 申请信息区块以只读表单展示：应用名称、开发者名称、同步范围类型、机构名称、是否包含下级、申请理由、白名单域名。
- 申请接口列表位于申请信息下方，列包含：接口名称、接口服务名、接口能力、数据敏感等级、接口分类、**最大调用量/日（按接口维度只读展示，以接口申请审批时填写为准，不可修改）**；列表分页展示，每页默认 5 条，可翻页查看全部接口。
...
申请信息区块（始终展示）：应用名称、开发者名称、申请时间、审批状态。

申请接口列表位于申请信息下方，列包含：接口名称、接口服务名、接口能力、数据敏感等级、接口分类；列表分页展示，每页默认 5 条，可翻页查看全部接口。
...
申请信息区块（始终展示）：应用名称、开发者名称、同步范围类型、机构名称、是否包含下级、申请理由、白名单域名、申请时间（**不展示审批状态**）。

申请接口列表位于申请信息下方，列包含：接口名称、接口服务名、接口能力、数据敏感等级、接口分类、最大调用量/日（按接口维度只读展示）；列表分页展示，每页默认 5 条，可翻页查看全部接口。
...
展示基础信息：企业LOGO（单图只读展示）、营业执照（多图只读展示，含文件名）；开发者名称、开发者类型、是否归属公海、统一社会信用代码、法人代表、管理员、管理员手机号、增值税一般纳税人、公司电话、营业期限、工商注册地址、详细地址、是否在门户展示、开发者简介。均为只读文本。

（2）编辑模式

编辑模式展示开发者完整信息，包含三部分：
- 基础信息：企业LOGO（图片上传）、营业执照（多图上传），开发者名称*、是否归属公海*、开发者类型*、统一社会信用代码*、法人代表*、管理员*、管理员手机号*、增值税一般纳税人*、公司电话*、营业期限*、工商注册地址*、详细地址*、是否在门户展示、开发者简介*。
- 财务信息：银行类别、开户行名称、开户行账号、账户用途（选填）。
- 开票信息：单位地址、纳税人识别号、电话号码、开户银行、银行账户、联行号（选填）。

保存时校验：必填项非空；管理员手机号须为 11 位数字；统一社会信用代码须为 18 位。校验不通过时提示具体缺失项；校验通过后不直接生效，而是提交「待桂教通超级管理员审批」申请，提示「编辑已提交，待桂教通超级管理员审批通过后生效」，并回到查看模式。

> **审批生效规则**：编辑提交后，企业信息维持原状，不立即生效，等待桂教通超级管理员审批。桂教通超级管理员在查看界面可对待审批申请「审批通过」（企业信息更新生效）或「审批不通过」（本次编辑不生效，企业信息维持原状）。
...
- **接口申请**：始终显示；点击打开「接口申请页（P28ApplyApi）」，表单仅包含应用名称（只读，当前选择的应用）与授权接口（多选，已选接口以分页表格展示，可单独删除）；提交后生成接口申请记录（P08），由桂教通超管审批。
- **授权申请**：始终显示；点击打开「数据授权申请页（P28Apply）」，表单包含应用名称（只读）、授权接口（多选，已选接口以分页可编辑表格展示，接口名称不可编辑、最大调用量/日按接口维度可编辑）、同步范围类型（学校/教育局可多选，勾选教育局时默认同步其下属机构，可改是否包含下级）、机构名称（多个用英文分号隔开）、是否包含下级、申请理由、免责声明（勾选同意）；提交后生成数据授权记录（P08Auth），由机构管理员审批。> 注：白名单字段已移至应用新增界面（P29），由可见性配置地址回填。
...
| 机构名称 | 文本输入 | 是（选择了同步范围类型时） | 完整机构名称，多个用英文分号（;）隔开；系统按机构名称精确匹配其机构管理员 |
| 申请理由 | 多行文本 | 是 | 填写申请用途与理由 |
| 免责声明 | 勾选同意 | 是 | 「我已阅读并同意」+ 可点击的《API接口调用免责声明》链接（点击弹窗展示声明全文，含数据使用范围、数据安全义务、保密义务、禁止行为、数据保留与销毁、合规要求、责任承担、其他八章），弹窗内也可一键勾选同意 |
...
| 授权范围 | 由"同步范围类型 + 机构名称 + 是否包含下级"拼出，数据来自「授权」弹窗（规则同 API 接口申请：教育局含/不含下属机构；学校仅机构名称）；未维护授权信息时为空 |
...
- **授权**：点击打开「授权」弹窗，展示并维护该客户端的授权信息；弹窗字段：客户端ID（只读展示）、客户端密钥（只读展示）、同步范围类型（可多选：学校 / 教育局）、机构名称（多行文本，完整机构名称，多个用英文分号 `;` 隔开）、是否包含下级（单选：是 / 否；当同步范围类型含"学校"时展示且必填默认是，含"教育局"时不展示——教育局默认同步其下属机构）。同步范围类型可同时勾选多种；勾选"教育局"时默认同步其下属机构，列表授权范围按所选各类型分别拼出（如"教育局：<机构名称>（含下属机构）；学校：<机构名称>"）。保存后更新列表「授权范围」字段。
- **信息查看**：点击打开「信息查看」弹窗，以表单文本只读展示该客户端新增/维护的全部信息（客户端ID、客户端密钥、授权类型、回调地址、登出后回调地址、访问令牌有效期、刷新令牌有效期、白名单、备注、使用状态、授权范围），样式同申请记录查看（P10）。若已维护授权信息，弹窗内额外展示「授权信息」区块（含同步范围类型、机构名称、是否包含下级）；若未维护授权信息，则不展示「授权信息」区块。

### （二）新增/编辑客户端授权（P31）

弹窗表单，字段与列表一致（不含"使用状态"与"授权范围"；客户端ID 与授权秘钥不在表单中填写，提交时系统自动生成），**除备注外均必填**。
...
> 同步范围类型、机构名称、是否包含下级三项不再在新增/编辑客户端授权（P31）中填写，改由列表操作列「授权」按钮弹窗维护；列表授权范围字段由该弹窗数据拼出，未维护时为空。同步范围类型枚举为学校/教育局（已删除机构），勾选「教育局」时默认同步教育局的下属机构（「是否包含下级」不展示）；勾选「学校」时展示「是否包含下级」（必填默认是，选择「否」仅同步该学校本级）。
...
本模块由桂教通超管为应用配置数据授权（应用名称、同步范围类型、机构名称、授权接口），支持新增/编辑/启用/停用，对应菜单：桂教通超管后台 → 授权管理 → 数据授权管理。包含数据授权管理列表（P34）一个页面，新增/编辑以弹窗形式完成。
...
- **新增数据授权**（工具栏按钮）：打开新增弹窗，字段如下：

| 字段 | 控件 | 必填 | 说明 |
|------|------|------|------|
| 应用名称 | 可搜索下拉框 | 是 | 根据应用名称模糊匹配应用，单选，最多匹配 30 个 |
| 同步范围类型 | 多选 | 是 | 枚举学校 / 教育局 |
| 机构名称 | 多行文本 | 是 | 多个机构用英文分号(;)隔开，如：南宁市教育局;南宁市第三中学 |
| 授权接口 | 弹窗接口列表选择 | 是 | 可多选，弹窗内支持按接口名称搜索，已选接口以标签展示可逐个移除 |

- **编辑**：打开编辑弹窗（同新增表单，预填当前值），可修改应用名称、同步范围类型、机构名称、授权接口。
...
| 同步范围类型 | string | 20 | 是 | — | 学校、教育局 | — | 用户多选 | 可多选 |
| 机构名称 | string | 500 | 是 | — | — | 多个用英文分号(;)隔开 | 用户输入 | 如：南宁市教育局;南宁市第三中学 |
...
- 未合作 → 合作中：确认合作（前提已确认联系），确认后开通开发者管理员登录（租户体系复用桂教通）。
- 合作中 → 已取消：取消合作（填写取消原因与时间，入口在开发者列表页），取消后开发者管理员手机号不可登录开发者平台。
- 已取消 → 合作中：重新合作（填写重新合作原因与时间），恢复开发者管理员登录权限。
...
| R02 | 确认联系前开发者数据可编辑，确认联系后不可编辑仅可查看 | 编辑操作 | 开发者管理 |
| R03 | 确认联系后才可进行确认合作操作 | 确认合作按钮 | 开发者管理 |
| R04 | 确认合作后开发者管理员可登录开发者平台（租户体系复用桂教通） | 确认合作完成 | 开发者管理 |
| R05 | 取消合作后开发者管理员手机号不可登录开发者平台 | 取消合作完成 | 开发者管理 |
...
| R19 | 数据授权管理（P34）新增/编辑必填：应用名称（可搜索模糊匹配，最多匹配30个）、同步范围类型（可多选，学校/教育局）、机构名称（多个用英文分号(;)隔开）、授权接口（弹窗接口列表多选） | 新增/编辑提交校验 | 数据授权管理 |
...
- AC-V-03：未确认联系的开发者可编辑；确认联系后编辑入口消失，只能查看。
- AC-V-04：未确认联系时确认合作不可操作；确认联系后可操作。
- AC-V-05：确认合作后开发者管理员可登录开发者平台（租户体系复用桂教通），合作状态变为合作中。
- AC-V-06：取消合作后开发者管理员手机号不可登录开发者平台，合作状态变为已取消。
- AC-V-07：重新合作后恢复登录权限，合作状态回到合作中。
- AC-V-08：邀请开发者后开发者管理员可收到邀请链接并注册，注册后开发者进入列表，合作状态为未合作。
...
- AC-D-01：列表展示应用名称、同步数据范围、机构名称（多个用英文逗号隔开）、授权的接口（多个用英文逗号隔开）、申请时间、申请人、状态。
- AC-D-02：新增/编辑弹窗应用名称为可搜索下拉框，按应用名称模糊匹配，单选，必填，最多匹配 30 个。
- AC-D-03：同步范围类型必填且可多选（学校/教育局）；机构名称必填，多个机构用英文分号(;)隔开。
- AC-D-04：授权接口通过弹窗接口列表选择，可多选，支持按接口名称搜索；未选择授权接口时阻止提交。
- AC-D-05：启用/停用需二次确认；停用后应用不可再调用被授权接口。
