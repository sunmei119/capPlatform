...
- 桂教通超管：能力管理（→开发者管理 P01、API接口管理 P04、接口分类管理 P04a、接口申请记录 P08、接口调用记录 P27、运营统计 P13）、授权管理（→客户端授权管理 P30）
- 开发者管理员：账号管理（→P16/P17）、接口中心（→P19）、应用管理（→P28、申请记录 P08）
...
列表展示当前筛选条件下的申请记录，列包含：接口名称（同一申请含多个接口时，接口名称以英文逗号隔开全部展示）、接口能力、应用ID、数据敏感等级、开发者名称、接口分类、申请时间、审批状态。支持按接口名称、应用ID、审批状态筛选。
...
页面供审批方查看接口申请详情并作出审核结论，含申请信息只读表单、申请接口列表与审批操作区。桂教通超管审批时可根据实际情况修改最大调用量/日。

（1）查看申请详情

页面布局顺序为：先展示"申请信息"区块，再展示"申请接口"列表。

- 申请信息区块以只读表单展示：应用ID、开发者名称、同步范围类型、机构名称、是否包含下级、**最大调用量/日（桂教通超管可编辑，机构管理员只读，默认带出申请值）**、申请理由、白名单域名。
- 申请接口列表位于申请信息下方，列包含：接口名称、接口服务名、接口能力、调用身份、数据敏感等级、**接口分类**；列表分页展示，每页默认 5 条，可翻页查看全部接口。

（2）提交审批

审批操作区填写：审核意见（必填，通过/不通过）、审核说明（文本框占满宽度，不通过时必填）、机构授权凭证（非必填，图片上传，支持多个，单个图片不可超过 10MB）。点击"提交审批"完成审核，系统记录超管调整后的最大调用量/日。
...
申请信息区块（始终展示）：应用ID、开发者名称、同步范围类型、机构名称、是否包含下级、最大调用量/日、申请理由、白名单域名、申请时间、审批状态。
...
本模块面向开发者管理员提供接口浏览与文档查看能力，对应设计基线模块 4.8。包含开发者接口列表（P19）、开发者接口文档（P20）两个页面，仅开发者管理员使用；接口申请能力已迁移至「开发者管理-应用管理（P28）」操作列「申请」入口（见页面 P28Apply）。机构管理员仍可在 API 接口管理（P04）发起批量申请（P21m）。
...
> 开发者接口中心不再提供「查看」与「申请」入口；接口申请能力统一收敛至「开发者管理-应用管理（P28）」操作列「申请」按钮（跳转应用接口申请页 P28Apply）。
...
页面为接口信息的只读查看页（文本只读模式），由 API 接口管理（P04 超管）操作列"查看"按钮打开（开发者接口列表 P19 的查看入口已移除），展示内容与新增/编辑接口（P05）一致。
...
页面为接口使用申请的提交页，本页（P21）作为应用接口申请页（P28Apply）的基础参考。> 注：开发者接口列表（P19）的"申请"与"新增申请（N）"入口已移除，接口申请能力统一收敛至「开发者管理-应用管理（P28）」操作列「申请」按钮，跳转 P28Apply（完整独立表单，字段详见下方 P28Apply 说明）。
...
> 开发者接口列表（P19）的"新增申请（N）"与"申请"入口已移除，本页面（P21m）现主要由接口列表（P04）"新增申请（N）"按钮、以及接口申请记录列表（P08）"重新提交"按钮进入。
...
3. 开发者基于已通过的应用，在 P28 操作列点击"申请"打开应用接口申请页（P28Apply，含授权接口多选）提交接口申请，并完成应用单点对接（SSO）。
...
| 操作 | 编辑（待审批/已驳回时显示）、查看（始终）、申请（始终）、联调完成（审批通过时显示） |
...
- **申请**：始终显示；点击打开「应用接口申请页（P28Apply）」，表单包含应用名称（只读）、授权接口（多选）、最大调用量/日、同步范围类型（机构/学校/教育局可多选，勾选机构时需选择是否包含下级）、机构名称（多个用英文分号隔开）、是否包含下级、申请理由、免责声明（勾选同意）；提交后进入审批流程。> 注：白名单字段已移至应用新增界面（P29），由可见性配置地址回填。
...
本页面由开发者管理-应用管理（P28）操作列「申请」按钮打开，用于为指定应用提交接口授权申请。表单包含以下字段：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| 应用名称 | 文本输入（只读） | 是 | 从 P28 携带，不可修改 |
| 授权接口 | 多选弹框 | 是 | 点击「选择授权接口」按钮弹出 API 列表弹框，支持多选；已选接口以标签形式展示，可单独移除 |
| 最大调用量/日 | 数字输入 | 是 | 填写每日最大调用次数上限 |
| 同步范围类型 | 复选框组（可多选） | 是 | 机构 / 学校 / 教育局；勾选「机构」时需选择下方「是否包含下级」 |
| 机构名称 | 文本输入 | 是（选择了同步范围类型时） | 完整机构名称，多个用英文分号（;）隔开；系统按机构名称精确匹配其机构管理员 |
| 是否包含下级 | 单选 | 是（同步范围类型含"机构"时） | 是 / 否（默认否）；仅表示数据范围是否包含该机构的下级单位 |
...
- 未填写机构名称时提示「请填写机构名称」
- 同步范围类型含「机构」但未选择是否包含下级时提示「请选择是否包含下级」
- 白名单存在空域名时提示「请填写完整的白名单域名」
- 未勾选免责声明时提示「请阅读并同意免责声明」

点击「提交申请」后弹窗二次确认，确认后提交成功并返回 P28。
...
| 授权范围 | 由"同步范围类型 + 机构名称 + 是否包含下级"拼出，数据来自「授权」弹窗（规则同 API 接口申请：机构含/不含下级；学校仅机构名称）；未维护授权信息时为空 |
...
- **授权**：点击打开「授权」弹窗，展示并维护该客户端的授权信息；弹窗字段：客户端ID（只读展示）、客户端密钥（只读展示）、同步范围类型（可多选：机构 / 学校 / 教育局）、机构名称（多行文本，完整机构名称，多个用英文分号 `;` 隔开）、是否包含下级（单选：是 / 否；当同步范围类型含"机构"时必填，其他情况隐藏）。同步范围类型可同时勾选多种；包含"机构"时需填写机构名称并选择是否包含下级，列表授权范围按所选各类型分别拼出（如"机构：<机构名称>（含/不含下级）；学校：<机构名称>；教育局：<机构名称>"）。保存后更新列表「授权范围」字段。
- **信息查看**：点击打开「信息查看」弹窗，展示该客户端新增/维护的全部信息（客户端ID、客户端密钥、授权类型、回调地址、登出后回调地址、访问令牌有效期、刷新令牌有效期、使用状态、授权范围）。若已维护授权信息，弹窗内额外展示「授权信息」区块（含同步范围类型、机构名称、是否包含下级）；若未维护授权信息，则不展示「授权信息」区块。

### （二）新增/编辑客户端授权（P31）

弹窗表单，字段与列表一致（不含"使用状态"与"授权范围"），**所有字段必填**。
...
> 同步范围类型、机构名称、是否包含下级三项不再在新增/编辑客户端授权（P31）中填写，改由列表操作列「授权」按钮弹窗维护；列表授权范围字段由该弹窗数据拼出，未维护时为空。
...
| 申请平台接口 | — | ✅（仅桂教通平台接口） | ✅ | 开发者管理员通过应用管理（P28）操作列"申请"跳转应用接口申请页（P28Apply，含授权接口多选）；桂教通超管流程同开发者管理员 |
| 审批接口申请 | ✅ | ✅ | — | 本机构/全平台 |
| 删除接口（开发者注册） | — | — | ✅ | 仅未上线状态 |
| 管理企业信息 | — | — | ✅ | — |
| 管理子账号 | — | — | ✅ | — |
