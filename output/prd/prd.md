...
- 应用统计卡片区：已授权应用数、授权接口数、应用调用总次数、应用调用成功率。
...
- 应用维度统计区（"应用维度统计"分组）：以"应用"为聚合维度展示以下四类统计：
  ...
  - 应用调用结果分布：展示调用成功、调用失败、调用成功率，并标注近7天累计调用次数与覆盖应用数。
...
- 存量指标（取当前实时值，不随所选时间范围变化）：接口总数（含敏感/非敏感分布）、开发者总数（含合作状态分布）、已授权应用数、接口分类分布、应用调用总次数、应用调用成功率。
...
- 应用维度统计（应用调用趋势 / 应用调用量 TOP5 / 应用授权接口数分布 / 应用调用结果分布）：以"应用"为聚合维度统计调用量 TOP5 应用的成功/失败调用量、调用量榜单、各应用授权接口数及调用结果分布；时间范围口径与接口维度一致。
...
- 应用维度：面向"授权应用"聚合，反映各应用调用表现。覆盖已授权应用数、应用调用总次数、应用调用成功率，以及应用调用趋势、应用调用量 TOP5、应用授权接口数分布、应用调用结果分布；用于识别高活跃与低成功率应用。
- 审批维度：面向"接口申请审批流程"聚合，反映审核效率与质量。覆盖申请审批状态分布（待审核/已通过/已驳回）与审核通过率（= 已通过 ÷（已通过 + 已驳回）× 100%）；用于监控待办与驳回风险。
...
### （九）数据回流管理（P36Table/P36/P36Form/P36Admin/P36Detail）
...
- **列表列**：序号、名称、应用名称、是否有接口（是/否，文本展示）、数据表、对接文档、同步状态（标签展示）、申请时间。
  - 数据表列：有接口时展示「接口对接」（对接文档独立成列）；无接口时展示同步数据表表key（多个以英文逗号隔开），如「tbl_student_base,tbl_score_detail」。
...
新开表单页，内容区最大宽度 960px，底部按钮居中展示。查看模式只读展示全部字段（含应用名称、开发者名称）。开发者新增时自动带出当前开发者名称。无接口场景下，查看模式逐卡片只读展示对应的「数据表」名称（含表key，如「学生基础信息表（tbl_student_base）」）及其「数据 json」完整内容；有接口场景只读展示对接文档与认证key（已加密）、对接域名。
...
- **提交校验**：名称必填；应用名称必选；有接口时至少上传一个对接文档、认证key 必填、对接域名若存在须以 http:// 或 https:// 开头；无接口时每个数据表卡片必须选择数据表；申请理由必填；免责声明必须勾选。
...
| 数据表管理（P36Table） | — | — | 本企业数据表 |
| 新增/编辑数据表（P36TableForm） | — | — | 本企业数据表 |
...
| 授权应用（应用授权） | — | ✅（全平台接口） | — | — |
| 批量授权接口 | — | ✅（全平台接口） | — | P04 勾选多个接口后点"批量授权"弹窗选择应用，仅非生态应用 |
| 添加授权 | — | ✅（全平台接口） | — | — |
| 移除授权 | — | ✅（全平台接口） | — | — |
| 批量移除授权 | — | ✅（全平台接口） | — | — |
| 接口停用/启用 | — | ✅（全平台接口） | — | 需二次确认 |
...
| 数据授权管理启用/停用 | — | ✅ | — | 二次确认 |
| 数据回流申请新增 | — | — | ✅ | 新开表单页提交，校验名称/应用名称/是否有接口/对接文档或同步范围/申请理由/免责声明；提交后状态为待审批并同步到超管列表 |
...
| 是否有接口 | string | 2 | 是 | 否 | 是、否 | — | 用户单选 | 控制对接文档/同步数据表展示 |
| 对接文档 | array | — | 是（有接口时） | — | — | PDF/PPT/Word/照片 | 用户上传 | 至少上传一个 |
| 同步数据表 | array | — | 是（无接口时） | — | — | — | 用户选择 | 数据表下拉单选 + 同步字段多选 |
| 申请理由 | string | 500 | 是 | — | — | 最多 500 字符 | 用户输入 | — |
| 免责声明 | boolean | — | 是 | 否 | — | — | 用户勾选 | 必须勾选 |
| 同步状态 | string | 10 | 是 | 待审批 | 待审批、同步中、待确认、已完成、已驳回 | — | 系统记录 | — |
| 审批说明 | string | 200 | 否 | — | — | — | 用户输入 | 驳回时必填 |
| 申请时间 | datetime | — | 是 | 当前时间 | — | YYYY-MM-DD HH:mm | 系统生成 | — |

### （十四）数据表实体（P36Table/P36TableForm）

| 字段 | 类型 | 长度 | 必填 | 默认值 | 枚举值 | 格式 | 业务来源 | 说明 |
|------|------|------|------|--------|--------|------|----------|------|
| 数据表名称 | string | 100 | 是 | — | — | — | 用户输入 | — |
| 数据表key | string | 64 | 是 | — | — | 英文标识 | 用户输入 | 全局唯一 |
| 数据表分类 | string | 50 | 否 | — | — | — | 用户选择 | — |
| 描述 | string | 200 | 否 | — | — | — | 用户输入 | — |
| 字段列表 | array | — | 是 | — | — | label（name） | 用户维护 | 数据回流同步字段来源 |
| 使用状态 | string | 10 | 是 | 未使用 | 未使用、使用中 | — | 系统记录 | 被数据回流引用后为使用中 |
| 上架状态 | string | 10 | 是 | 未上架 | 未上架、已上架 | — | 系统记录 | 仅已上架数据表可被数据回流选用 |
| 创建时间 | datetime | — | 是 | 当前时间 | — | YYYY-MM-DD HH:mm | 系统生成 | — |
...
状态集合：待审批、同步中、待确认、已完成、已驳回。

- 待审批 → 同步中：超管审批通过。
- 待审批 → 已驳回：超管审批驳回（审批说明必填）；已驳回记录可编辑后重新提交。
- 同步中 → 待确认：开发者「同步完成」（仅是否有接口为「否」时展示，二次确认）。
- 同步中 → 已完成：超管「同步完成」（仅同步中且有接口时展示，二次确认）。
- 待确认 → 已完成：超管「同步确认」（二次确认）。

### （八）数据表状态机（P36Table/P36TableForm）

状态集合：未上架、已上架。

- 未上架 → 已上架：上架；已上架数据表可被数据回流申请选用。
- 已上架 → 未上架：下架需二次确认；下架仅限未被数据回流引用（使用状态为「未使用」）的数据表，已下架数据表不可再被数据回流申请选用。
...
- AC-RFL-01：开发者列表（P36）数据表列有接口时展示「接口对接」，无接口时展示同步数据表表key（多个以英文逗号隔开）；对接文档列有接口时展示对接文档名称列表且可点击弹窗预览，无接口时展示「—」。
- AC-RFL-02：新增数据回流（P36Form）：名称必填、申请理由必填（最多 500 字符）、免责声明必须勾选；有接口时至少上传一个对接文档，无接口时每个数据表卡片必须选择数据表且至少选一个同步字段。
- AC-RFL-03：提交后状态为「待审批」，并同步生成一条待审批数据到超管数据回流列表（P36Admin）。
- AC-RFL-04：超管「审批」仅待审批可操作（通过→同步中，驳回→已驳回，驳回时审批说明必填）；开发者「同步完成」仅是否有接口为「否」且同步中时可点击，二次确认后状态变「待确认」；超管「同步完成」仅同步中且有接口时展示，二次确认后状态变「已完成」；超管「同步确认」仅待确认时展示，二次确认后状态变「已完成」。
- AC-RFL-05：数据回流明细（P36Detail）为超管台账，按接口维度一行一条记录；列表含数据表key、接口名称（无接口为同步数据表名，有接口为「平台接口回流（对接文档）」）、同步数据量、回流状态（成功/失败标签）、失败原因（失败红色展示）、最近同步时间（未同步「—」）。
- AC-RFL-06：「手动同步」仅是否有接口为「是」且回流状态为「失败」时展示，二次确认后手动调接口拉取数据并更新为成功；「查看」跳转回流明细查看（P36DetailView）只读页，按明细行类别展示与该行一致的查看信息（接口回流：接口名称与对接文档预览；数据表回流：数据表名称（key）、同步字段、数据 json）。

### （十二）数据表（P36Table/P36TableForm）

- AC-TBL-01：数据表管理列表（P36Table）展示数据表名称、数据表key、数据表分类、描述、使用状态（未使用/使用中）、上架状态（未上架/已上架）、创建时间。
- AC-TBL-02：新增/编辑数据表（P36TableForm）：数据表名称必填；数据表key 必填且全局唯一（英文标识）；字段列表至少一条，以 label（name）形式维护。
- AC-TBL-03：上架/下架需二次确认；下架仅限使用状态为「未使用」的数据表；仅已上架数据表可被数据回流申请选用。
