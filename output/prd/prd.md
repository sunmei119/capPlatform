...
2. 点击新增，填写客户端ID、回调地址、白名单、备注等信息（授权类型不展示，默认 authorization_code 授权码方式），提交后系统自动生成授权秘钥，建立客户端授权记录。
...
本模块由桂教通超管维护 OAuth 客户端与平台的授权关联关系，对应菜单：桂教通超管后台 → 授权管理 → 客户端授权管理。包含客户端授权管理列表（P30）与新增/编辑客户端授权（P31）两个页面；P31 同时作为「授权」操作打开的新开授权配置表单页。
...
| 授权类型 | 新增/编辑固定为 authorization_code（授权码），不提供选择；列表保留历史数据原类型 |
| 回调地址 | redirect_uri |
...
| 客户端授权信息查看 | — | ✅ | — | P30 列表操作列「信息查看」弹窗只读展示 |
...
| 接口类型 | string | 10 | 是 | — | 个人、机构 | — | 用户单选 | 接口的适用主体类型 |
...
- AC-D-02：可新增子账号（员工姓名、员工手机号必填，手机号校验 11 位格式），删除子账号需二次确认。
...
- AC-CLT-03：授权配置（P31 新开表单页）：客户端ID 与客户端密钥只读展示；是否全部数据选「是」时不展示同步范围类型、学校名称、机构名称；是否全部接口选「是」时不展示授权接口；授权截止时间不可选择今天之前、只能选择三年内。
