...
| 业务数据表关联 | 业务树维护（新增/编辑/停用）、业务与数据表关联管理（关联/移出），点击业务节点查看其关联的数据表 | 桂教通超管 |
...
1. 桂教通超管在数据回流管理（P36Admin）点击"新增"，选择应用名称、目标开发者（可搜索下拉）、对接方式（开发者提供接口/开发者主动推送）、推送方式（全量/增量）并填写相关信息提交。对接方式为"开发者提供接口"时填写认证key（选填）与对接域名（选填），提交后状态为"待对接"；对接方式为"开发者主动推送"时填写唯一字段（单个字段名，表单级别）并选择同步数据表（选择数据表key，自动带出该数据表在数据表管理中配置的表json，本页不编辑表json）与认证key（选填），提交后状态为"联调中"。
...
- **名称**：文本输入，必填，最多 50 字符。
- **应用名称**：下拉选择（可搜索），必填；根据应用名称模糊匹配，最多展示 30 个匹配应用。
- **开发者名称**：下拉选择（可搜索），必填；查询平台所有开发者，单选。
- **对接方式**：单选（开发者提供接口/开发者主动推送，默认开发者提供接口）。
- **推送方式**：单选（全量/增量），必填；两种对接方式下均需选择。
  - 选「开发者提供接口」：展示认证key（文本输入，选填，加密存储与展示）与对接域名（文本输入，选填，需以 http:// 或 https:// 开头）。对接文档区域不展示（由开发者后续上传）。
...
- **提交校验**：名称必填；应用名称必选；开发者名称必选；推送方式必选；对接方式为「开发者主动推送」时唯一字段必填（最多 64 字符，仅允许单个字段名）、每个数据表卡片必须选择已配置合法表json 的数据表key（表json 自动带出、只读）；对接域名若存在须以 http:// 或 https:// 开头；申请理由必填；免责声明必须勾选。
...
- **列表列**：数据表key、数据库表名称、业务场景说明、状态（启用/停用，标签展示）、创建时间。
- **操作列**：
  - 新增：工具栏「新增」按钮，打开新增弹窗，填写数据表key（文本输入，必填，全局唯一）、数据库表名称（文本输入，必填）、业务场景说明（文本输入，选填）；提交校验数据表key 唯一，重复时提示「数据表key 已存在」；新增记录默认状态为「启用」。
  - 编辑：行内「编辑」按钮，打开编辑弹窗并回显当前值，字段与新增一致；数据表key 编辑时不可修改（只读展示）。
...
桂教通超管在本页维护平台业务树，并把数据表管理（P37）中启用状态的数据表关联到具体业务节点，形成「业务—数据表」归属关系。对应菜单：桂教通超管后台 → 授权管理 → 业务数据表关联。页面布局与交互同接口能力管理（P35）：左侧树形业务树，右侧为选中业务关联的数据表列表。

**（1）业务树（左侧）**

左侧以树形（el-tree）展示平台全部业务节点，节点仅展示业务名称，不展示状态标签；停用的业务名称置灰展示。

- 树上方提供「新增业务」按钮（新增顶级业务）与搜索框：搜索框支持按业务名称 / 业务编码模糊筛选业务树。
- 鼠标经过节点时显示「新增」「编辑」「停用」按钮：
  - 新增：以该节点为父级新增子业务，打开新增弹窗并预选父级业务。
  - 编辑：打开编辑弹窗并回显当前业务信息。
  - 停用/启用：点击弹出二次确认；停用确认提示「停用后该业务不可再关联新的数据表，已有数据不受影响」，确认后状态在启用、停用间切换。
- 点击业务节点后该节点高亮，右侧联动展示其关联的数据表列表。

**（2）新增/编辑业务（弹窗）**

弹窗字段：

- 父级业务：树选择，可不选（不选则为顶级业务）；新增子业务时预选当前节点。
- 业务名称：文本输入，必填，最多 50 字符。
- 业务编码：文本输入，必填，全局唯一，最多 50 字符；重复时提示「业务编码已存在」。
- 排序：数字输入，选填，默认 0。

提交校验：业务名称为空提示「请输入业务名称」；业务编码为空提示「请输入业务编码」；业务编码重复提示「业务编码已存在」。新增业务默认状态为「启用」。

**（3）关联数据表列表（右侧）**

右侧展示当前选中业务已关联的数据表，列包含：数据表key、数据库表名称、业务场景说明、表json（悬浮查看完整内容）、数据表状态（启用/停用，标签展示）。列表顶部展示关联数量与「新增关联」按钮，操作列提供「移出」。

- 未选择业务时提示「请先在左侧选择业务」。
- 无关联数据表时显示「暂无关联数据表」。
- 当前业务为「停用」状态时，「新增关联」按钮置灰不可点击（点击提示「停用的业务不可关联数据表」），已有数据表关联保留不受影响。

**（4）关联数据表**

点击「新增关联」打开弹窗，顶部提供「数据表key」「数据库表名称」两个查询条件（均支持模糊匹配，可清空后查询，提供「查询」「重置」），下方展示启用状态且未被当前业务关联的数据表（数据表key、数据库表名称、业务场景说明），支持多选（复选框）；翻页或更换查询条件后已勾选的数据表保持选中。确认后将所选数据表加入当前业务的关联列表。若无符合条件的数据表，列表展示空数据。

- 未选择任何数据表时点击确定提示「请至少选择一个数据表」。

**（5）移出数据表**

点击行内「移出」，弹出二次确认「确认将数据表「xxx」移出业务「xxx」？」，确认后解除该业务与该数据表的关联，列表实时刷新。
...
| 新增数据表（超管） | — | ✅ | — | P37 工具栏「新增」打开弹窗，填写数据表key（必填，唯一）、数据库表名称（必填）、业务场景说明（选填）、表json（必填，可编辑文本框输入 + 「校验」按钮校验为合法 JSON 对象），新增默认启用 |
...
| 新增业务（超管） | — | ✅ | — | P38 弹窗新增：父级业务（树选择，可不选）、业务名称（必填，≤50字符）、业务编码（必填，全局唯一）、排序（选填）；新增默认启用 |
| 编辑业务（超管） | — | ✅ | — | P38 业务树节点「编辑」打开弹窗，回显当前值，父级业务/业务名称/业务编码/排序可改 |
| 停用/启用业务（超管） | — | ✅ | — | P38 业务树节点「停用」/「启用」，二次确认；停用后该业务不可再关联新的数据表，已有数据表关联不受影响 |
| 新增关联数据表（超管） | — | ✅ | — | P38 右侧「新增关联」弹窗多选，候选为启用状态且未被当前业务关联的数据表，支持按数据表key/数据库表名称模糊查询；停用业务的按钮置灰不可关联 |
| 移出数据表（超管） | — | ✅ | — | P38 关联数据表列表操作列「移出」，二次确认后解除该业务与数据表的关联 |
...
| 字段 | 类型 | 长度 | 必填 | 默认值 | 枚举值 | 格式 | 业务来源 | 说明 |
|------|------|------|------|--------|--------|------|----------|------|
| 业务名称 | string | 50 | 是 | — | — | — | 用户输入 | 业务节点名称，在业务树展示 |
| 业务编码 | string | 50 | 是 | — | — | 英文标识 | 用户输入 | 全局唯一，标识一个业务节点 |
| 父级业务 | string | — | 否 | — | — | — | 用户选择 | 业务树父节点；不选则为顶级业务 |
| 排序 | number | — | 否 | 0 | — | 非负整数 | 用户输入 | 同级业务节点展示顺序 |
| 业务状态 | string | 10 | 是 | 启用 | 启用、停用 | — | 系统记录 | 新增默认启用；停用后不可再关联新的数据表，已有数据表关联不受影响 |
| 关联数据表 | array | — | 否 | 空 | — | 数据表key 数组 | 用户选择 | 该业务关联的数据表；候选为数据表管理（P37）中启用状态的数据表 |
...
| R37 | 数据表管理（P37）：桂教通超管在授权管理 → 数据表管理维护业务场景数据表的 key、数据库表名称与表json；列表展示数据表key、数据库表名称、业务场景说明、状态（启用/停用）、创建时间；新增弹窗填写数据表key（必填，全局唯一，重复时提示「数据表key 已存在」）、数据库表名称（必填）、业务场景说明（选填）、表json（必填，可编辑文本框输入 + 「校验」按钮校验为合法 JSON 对象），新增默认启用；编辑弹窗数据表key 只读、其余可改（含表json 维护）；启用/停用需二次确认，停用确认提示「停用后新增的数据回流将不可选择该数据表，已有数据不受影响」；启用状态或已被数据回流引用的数据表禁止删除（按钮置灰），仅停用且未被引用的数据表可删除，删除需二次确认（「确认删除该数据表？删除后不可恢复」）；P37 启用状态的数据表作为 P36Form 中数据表key 下拉选择的数据来源，选择后自动带出该数据表配置的表json | 新增、编辑、启用/停用、删除、列表查询 | 数据表管理 |
...
| R43 | 业务数据表关联（P38）采用左右分栏（布局与交互同接口能力管理 P35）：左侧树形业务树仅展示业务名称（不显示状态标签，停用业务置灰），支持按业务名称/编码筛选、新增（顶级/子级）、编辑、停用（二次确认，停用后不可再关联新的数据表，已有关联不受影响）；业务名称必填（≤50字符）、业务编码必填且全局唯一；点击业务节点后右侧展示该业务关联的数据表，「新增关联」弹窗仅可选启用状态且未被当前业务关联的数据表，弹窗顶部支持按数据表key/数据库表名称模糊查询（含查询/重置，勾选结果翻页与换查询条件后保留），停用业务不可关联（按钮置灰）；「移出」需二次确认 | 业务树维护、业务-数据表关联/移出 | 业务数据表关联 |
...
- AC-TBL-01：桂教通超管在授权管理 → 数据表管理可查看全部数据表记录，列表展示数据表key、数据库表名称、业务场景说明、状态（启用/停用，标签展示）、创建时间。
- AC-TBL-02：新增弹窗填写数据表key（必填，全局唯一）、数据库表名称（必填）、业务场景说明（选填）、表json（必填，可编辑文本框输入，点「校验」或提交时未填写/非合法 JSON 对象均提示并阻止提交）；数据表key 重复时提示「数据表key 已存在」并阻止提交；新增记录默认状态为「启用」。
- AC-TBL-03：编辑弹窗回显当前值，数据表key 只读不可修改，数据库表名称、业务场景说明与表json 可编辑（表json 经可编辑文本框输入 + 「校验」确认为合法 JSON 对象）。
...
- AC-BIZ-01：桂教通超管在授权管理 → 业务数据表关联进入，页面为左右分栏（布局与交互同接口能力管理 P35）：左侧树形业务树（仅展示业务名称，不显示状态标签，停用业务置灰，支持按业务名称/业务编码筛选），右侧为选中业务关联的数据表列表；未选择业务时右侧提示「请先在左侧选择业务」。
- AC-BIZ-02：左侧业务树支持新增（工具栏「新增业务」新增顶级业务、节点操作「新增」新增子业务）、编辑、停用/启用；鼠标经过节点时显示三个操作按钮；停用/启用需二次确认，停用确认提示「停用后该业务不可再关联新的数据表，已有数据不受影响」。
- AC-BIZ-03：新增/编辑业务弹窗含父级业务（树选择，可不选；新增子业务时预选当前节点）、业务名称（必填，≤50字符）、业务编码（必填，≤50字符，全局唯一，重复提示「业务编码已存在」）、排序（选填）；业务名称为空提示「请输入业务名称」，业务编码为空提示「请输入业务编码」；新增业务默认启用。
- AC-BIZ-04：点击业务节点后右侧展示该业务关联的数据表（数据表key、数据库表名称、业务场景说明、表json[悬浮查看完整内容]、数据表状态[标签]），顶部展示关联数量与「新增关联」按钮；无关联数据表时展示「暂无关联数据表」。
- AC-BIZ-05：「新增关联」弹窗顶部提供「数据表key」「数据库表名称」模糊查询条件（含「查询」「重置」），列表仅展示启用状态且未被当前业务关联的数据表，支持多选，翻页/换查询条件后已勾选项保留；未选择时点击确定提示「请至少选择一个数据表」；当前业务为停用时按钮置灰不可关联，已有关联保留。
- AC-BIZ-06：行内「移出」二次确认（「确认将数据表「xxx」移出业务「xxx」？」）后解除该业务与数据表的关联，列表实时刷新。
