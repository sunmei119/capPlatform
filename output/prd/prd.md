...
3. 客户端授权管理中存在「机构申请」类型的记录（由机构组织同步申请经「审批管理 → 组织同步审批（P41）」审批通过后自动生成，含客户端ID与客户端秘钥），本列表不提供审批操作；平台授权类型操作列为编辑/启用/停用/数据授权/接口授权/查看，机构申请类型操作列为启用/停用/查看/接口授权。
...
- **查看**：点击打开「客户端信息查看」新界面（P30View，非弹窗），以表单文本只读展示该客户端申请/维护的全部信息（客户端ID、客户端密钥、客户端类型、回调地址、登出后回调地址[仅平台授权展示]、访问令牌有效期、刷新令牌有效期、白名单、备注、启用状态、授权范围），样式同申请记录查看（P10）。其中「客户端秘钥」默认脱敏展示（保留前4位与后4位，中间以 `****` 代替），以蓝色字体显示，右侧小眼睛图标点击可在「脱敏/明文」间切换，点击秘钥串可复制完整秘钥。若已维护授权信息，界面内额外以列表形式展示「数据授权」与「接口授权」：
  - 「数据授权」列表（同步范围类型 / 学校名称 / 机构名称 / 授权截止时间 / 授权接口，多个机构拆多行）：仅客户端类型为「平台授权」时展示；客户端类型为「机构申请」时不展示该区块。
...
| R26 | 客户端数据授权配置（P31）与接口授权配置（P31B）为两个独立的新开表单页。P31 数据授权配置：是否全部数据（单选是/否，选「是」时授权数据为全区数据，不展示同步范围类型、学校名称与机构名称；选「否」时展示）、同步范围类型勾选「学校」展示「学校名称」、勾选「教育局」展示「机构名称」（多行文本框，英文分号隔开）、授权截止时间（不可选择今天之前的时间，只能选择当前时间的三年内的时间）、授权接口（弹窗多选，必填，选择该数据授权关联的接口）；不包含「是否包含下级」。P31B 接口授权配置：是否全部接口（单选是/否，选「是」时不展示授权接口、授权全部接口；选「否」时展示）、授权接口（弹窗多选） | 数据授权/接口授权表单展示与提交校验 | 客户端授权 |
...
- AC-CLT-03：数据授权配置（P31 新开表单页）由 P30 操作列「数据授权」跳转（仅平台授权类型展示）；客户端ID 与客户端密钥只读展示；是否全部数据选「是」时不展示同步范围类型、学校名称、机构名称，授权截止时间不可选择今天之前、只能选择三年内；授权接口必填（弹窗多选）。接口授权配置（P31B 新开表单页）由 P30 操作列「接口授权」跳转（平台授权与机构申请类型均展示）；是否全部接口选「是」时不展示授权接口。
