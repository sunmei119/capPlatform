...
- 申请接口列表位于申请信息下方，列包含：接口名称、接口服务名、接口平台、数据敏感等级、接口分类、**最大调用量/日（按接口维度只读展示，以接口申请审批时填写为准，不可修改）**、**文档链接（蓝色超文本展示，点击新窗口打开对应接口文档）**；列表分页展示，每页默认 5 条，可翻页查看全部接口。
...
| 授权接口 | 多选弹框 | 是 | 点击「选择授权接口」按钮弹出 API 列表弹框，支持多选；弹框内仅展示**本开发者名下应用已申请审批通过的接口**（即接口申请经「审批管理 → 接口申请审批」审批通过后的接口；接口申请未通过审批的接口不可选、不展示）；已选接口以分页可编辑表格展示（接口名称不可编辑，最大调用量/日按接口维度可编辑），可单独删除 |
...
- 操作列含四个动作（每个动作均针对当前行应用）：
  - **查看**：打开授权记录查看页（P28AuthView，见下），以只读形式展示该应用的全部接口授权记录与数据授权记录（界面同取消授权页但不含操作列）。
...
由 P28All 操作列「查看」进入（携带当前应用），界面与取消授权页（P28AuthCancel）同构但**不含「操作」列**，仅用于只读查看该应用的全部授权记录。

- 页头信息栏展示应用名称与所属厂商，提示「以下为该应用的全部授权记录（接口授权记录 + 数据授权记录），仅用于查看；如需取消 / 启用授权，请在「取消授权」页面操作」。
- **接口授权记录 Tab**：接口名称 / 接口服务名 / 数据敏感等级 / 最大调用量/日 / 授权截止时间 / 状态，分页（每页 5 条）。
- **数据授权记录 Tab**：范围类型 / 机构名称 / 授权的接口 / 授权截止时间 / 状态，分页（每页 5 条）。
- 若无记录，对应 Tab 展示空态提示「暂无接口授权记录 / 暂无数据授权记录」。
...
- **数据授权**：仅平台授权类型展示。点击跳转数据授权配置页（P31，新开表单页），展示并维护该客户端的数据授权信息（客户端ID/密钥只读）。字段：
  ...
  - **授权截止时间**（时间控件，必填）：不可选择当前时间之前的时间，只能选择当前时间的三年内的时间。
  - **授权接口**（弹窗多选，必填）：选择该数据授权关联的接口，展示同接口授权配置页（P31B）的选择方式。
...
- AC-R-05：审批页与查看页展示申请人应用的白名单配置（前缀 + 域名，来自 P29 应用可见性配置）。
...
- AC-AP-06：接口授权（P28ApiAuth）：应用名称与所属厂商只读；授权接口候选为平台开放（对外可见）且「是否需要授权=是」的接口（可按名称/服务名搜索多选），已选接口以列表展示（接口名称、最大调用量/日[数据输入框可编辑]、删除）；授权截止时间必填（不可选今天之前、只能三年内）；至少选择一项接口方可保存；保存即生效（无审批）并在取消授权-接口授权记录 Tab 可见。
- AC-AP-07：数据授权（P28DataAuth）：应用名称预选只读；同步范围类型可多选（学校/教育局，勾选「学校」展示「学校名称」、勾选「教育局」展示「机构名称」，必填多行文本框，多个用英文分号(;)隔开）、授权接口（弹窗多选必填）、授权截止时间（必填，不可选择今天之前的时间，只能选择当前时间的三年内的时间）；应用级数据授权统一在应用管理本页按应用维护，与客户端授权（P30）等流程相互独立；保存后新增应用级数据授权记录——一次录入多个学校/机构（英文分号隔开）按机构拆分，一个机构生成一条记录；各条记录可在取消授权-数据授权记录 Tab 可见并独立置停用/启用。
...
- AC-AP-09：查看授权记录（P28AuthView）：操作列「查看」打开只读页面，界面与取消授权页（P28AuthCancel）同构但不含「操作」列；两 Tab（接口授权记录/数据授权记录）分页展示该应用的全部授权记录；如需取消/启用授权须在「取消授权」页面操作。
...
- AC-CLT-03：数据授权配置（P31 新开表单页）由 P30 操作列「数据授权」跳转（仅平台授权类型展示）；客户端ID 与客户端密钥只读展示；是否全部数据选「是」时不展示同步范围类型、学校名称、机构名称，授权截止时间不可选择今天之前、只能选择三年内；授权接口必填（弹窗多选）。接口授权配置（P31B 新开表单页）由 P30 操作列「接口授权」跳转（平台授权与机构申请类型均展示）；是否全部接口选「是」时不展示授权接口；选「否」时授权接口弹窗多选，已选接口以列表展示（接口名称、最大调用量/日[次/日，可编辑]、删除）。
